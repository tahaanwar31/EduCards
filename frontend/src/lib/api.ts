...
export async function generateQuiz(flashcards: { question: string; answer: string }[]): Promise<{ questions: any[]; aiPowered: boolean }> {
  const res = await fetch(`${API_URL}/quiz/generate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ flashcards })
  });
  return handleResponse(res, 'Failed to generate quiz');
}
